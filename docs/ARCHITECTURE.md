...
### 6.1 Personalized Prediction Flow (NEW)

The system now includes a learning feedback loop that improves prediction accuracy over time:
...
└──────────────────────────────────────────────────────────────────┘
    ...
    ┌─────────────────┐
    │   Birth Data    │  User provides birth datetime
    │   Collection    │  (BirthDataPrompt component)
    └────────┬────────┘
             │ Natal longitudes computed
             ▼
    ┌─────────────────┐
    │  Natal 16D      │  compute16DFromBirthData()
    │  Vector         │  Cached in localStorage (rop_natal_16d)
    └────────┬────────┘
             │
             ▼
    ┌─────────────────────────────────────────────────────────────┐
    ...
    │         │ compare                              │            │
    │  ┌─────────────┐                              │            │
    │  │ Transit 16D │ ◄── Current sky positions   │            │
    │  └─────────────┘                              │            │
    │                                               ▼            │
    │                              ┌─────────────────────────────┐│
    │                              │   PredictionCard Display    ││
    │                              │   (BEFORE check-in)         ││
    │                              └──────────────┬──────────────┘│
    │                                             │               │
    └─────────────────────────────────────────────│───────────────┘
    ...
    │                    CALIBRATION SYSTEM                        │
    │                                                              │
    │  storeFeedback() ─► rop_prediction_feedback (90 days)       │
    │         │                                                    │
    │         ▼                                                    │
    │  ┌─────────────────┐                                        │
    │  │ Bias Detection  │  Tracks if predictions run high/low   │
    │  └────────┬────────┘                                        │
    │           │                                                  │
    │  ┌────────▼────────┐                                        │
    │  │ Dimension       │  Which transits affect user most?      │
    │  │ Sensitivities   │  (Moon, Mercury, Venus, etc.)          │
    │  └────────┬────────┘                                        │
    │           │                                                  │
    │  ┌────────▼────────┐                                        │
    │  │ calibratePrediction() │  Adjusts future predictions     │
    │  └─────────────────┘                                        │
    │                                                              │
    └──────────────────────────────────────────────────────────────┘
```
...
- `src/lib/prediction-calibration.ts` - Feedback storage & learning
...
└────────┬────────┘
         ...
         ▼
┌─────────────────┐
│ PredictionCard  │  2. Show predicted kappa (if birth data exists)
└────────┬────────┘
         │ User clicks "Start Check-in"
         ▼
┌─────────────────┐
│  Check-in Form  │  3. 6 questions (coherence, energy, focus, embodiment)
└────────┬────────┘
         │ User submits scores
         ▼
┌─────────────────┐
│  Engine Service │  4. Compute κ, RU, failure mode
└────────┬────────┘
         │ Results computed
         ▼
┌─────────────────┐
│ PredictionComp  │  5. Compare predicted vs actual (if available)
└────────┬────────┘     Store feedback for calibration
         │
         ▼
┌─────────────────┐
│  Voice Service  │  6. Generate 3 voices
└────────┬────────┘
         │ kasra, river, sol texts
         ▼
┌─────────────────┐
│  Memory Service │  7. Store check-in + forecast
└────────┬────────┘
         │
         ▼
┌─────────────────┐
│ BirthDataPrompt │  8. If no birth data, prompt after 2nd check-in
└────────┬────────┘
         │
         ▼
┌─────────────────┐
│  Results View   │  9. Render forecast in user's mode
└─────────────────┘
```
